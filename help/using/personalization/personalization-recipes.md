...
Schließen Sie den mit Bindestrichen versehenen Schlüssel in einen PQL-Ausdruck ein: `{%= profile.events.\`order-total\` > 100 %&rbrace;`. Backticks are not supported in plain Handlebars interpolation — use a `{% let %}&grave; Variable als Zwischenschritt, falls erforderlich.
