...
Schließen Sie den mit Bindestrichen versehenen Schlüssel in einen PQL-Ausdruck ein: `{%= profile.events.\`order-total\` > 100 %}`. Backticks are not supported in plain Handlebars interpolation — use a `{% let %}` Variable als Zwischenschritt, falls erforderlich.
