...
| `name` | Ein String, der zum Abgleich der xdm:name-Eigenschaft der Entitäten dient. Die Zeichenfolge wird exakt abgeglichen (mit Groß-/Kleinschreibung), es können jedoch Platzhalter genutzt werden. Die Parameter `id` und `name` können nicht zusammen verwendet werden. | `Sales and Promotions Placement` |
