...
# Angebote mithilfe der [!DNL Batch Decisioning] API
...
Mit dem [!DNL Batch Decisioning] API können Sie einen Datensatz mit den besten Angeboten für alle Profile in einem Adobe Experience Platform-Segment für einen Entscheidungsbereich ausfüllen. So kann beispielsweise eine Organisation die Batch-Entscheidungsfindung ausführen, damit sie Angebote an einen Anbieter eines Nachrichtenversands senden kann. Diese Angebote werden dann als Inhalt verwendet, der für die Batch-Nachrichtenübermittlung an dasselbe Benutzersegment gesendet wird.

Dazu würde die Organisation:

* Führen Sie die [!DNL Batch Decisioning] API, die zwei Anfragen enthält:

   1. A **Batch-GET-Anfrage** , um den Status der Batch-Arbeitslast zu erhalten.

   2. A **Batch-POST-Anfrage** , um eine Arbeitslast für die Stapelverarbeitung der Angebotsauswahl zu starten.
...
Gehen Sie wie folgt vor, um Ihre Entscheidungen vorzubereiten:

* Um ein Entscheidungsergebnis zu exportieren, erstellen Sie einen Datensatz mit dem Schema &quot;ODE-Entscheidungsereignisse&quot;.
...
Für alle Batch Decisioning-Anforderungen sind die folgenden zusätzlichen Header erforderlich:
...
| Eigenschaft | Beschreibung | Beispiel |
| -------- | ----------- | ------- |
| `xdm:segmentIds` | Die eindeutige Kennung des Segments. Sie darf nur einen Wert enthalten. | `609028e4-e66c-4776-b0d9-c782887e2273` |
| `xdm:dataSetId` | Der Ausgabedatensatz, in den Entscheidungsereignisse geschrieben werden können. | `6196b4a1a63bd118dafe093c` |
| `xdm:propositionRequests` | Ein Wrapper, der die placementId und activityId enthält |  |
...
Siehe Abschnitt [Dokumentation zur Entscheidungsverwaltung](../../get-started/starting-offer-decisioning.md) für einen Überblick über die wichtigsten Konzepte.
...
Die End-to-End-Zeit für jede Batch-Entscheidung ist die Dauer von dem Zeitpunkt, zu dem die Arbeitslast erstellt wird, bis zum Entscheidungsergebnis, das im Ausgabedatensatz verfügbar ist. Die Segmentgröße in der Payload der POST-Anfrage ist der Hauptfaktor, der sich auf die End-to-End-Batch-Entscheidungszeit auswirkt. Im Folgenden finden Sie einige Beobachtungen zu verschiedenen Segmentgrößen:

* Segmentgröße &lt;= 10K: 6 Minuten
* Segmentgröße &lt;= 1M: 10 Minuten
* Segmentgröße &lt;= 15M: 75 Minuten

## Einschränkungen {#limitations}

Die folgenden Einschränkungen gelten für [!DNL Batch Decisioning] API:

* **EinzelBatch-Auftrag**: Derzeit kann pro Datensatz immer nur ein einzelner Batch-Auftrag ausgeführt werden. Alle anderen Anfragen antworten mit HTTP 429 (zu viele Anfragen).
...
In diesem API-Handbuch haben Sie nach dem Arbeitsbelastungsstatus gesucht und Angebote mithilfe der Variablen [!DNL Batch Decisioning] API. Weitere Informationen finden Sie unter [Übersicht über das Entscheidungs-Management](../../get-started/starting-offer-decisioning.md).
