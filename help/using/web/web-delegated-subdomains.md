...
>title="Delegieren einer Web-Subdomain"
>abstract="Wenn Sie Inhalte aus Adobe Experience Manager Assets Essentials zu Ihren Web-Erlebnissen hinzufügen, müssen Sie die Subdomain einrichten, die zum Veröffentlichen dieses Inhalts verwendet wird. Wählen Sie unter den bereits an Adobe delegierten Subdomains aus oder konfigurieren Sie eine neue Subdomain."
...
## Zugreifen auf und Verwalten von Web-Subdomains {#access-web-subdomains}

1. Navigieren Sie zu **[!UICONTROL Administration]** > **[!UICONTROL Kanäle]** Menü und wählen Sie **[!UICONTROL Webkonfiguration]** > **[!UICONTROL Web-Subdomains]**. Alle mit der aktuellen Sandbox eingerichteten Subdomains werden angezeigt.

   ![](assets/web-access-subdomains.png)

1. Sie können nach dem Benutzer filtern, der jede Subdomain oder einen Delegationsstatus zugewiesen hat (**[!UICONTROL Entwurf]**, **[!UICONTROL Verarbeitung]**, **[!UICONTROL Erfolg]** oder **[!UICONTROL Fehlgeschlagen]**).
...
1. Wählen Sie die **[!UICONTROL Delegierte Subdomain verwenden]** Option aus der **[!UICONTROL Konfigurationstyp]** und wählen Sie eine zugewiesene Subdomain aus der Liste aus.
...
>abstract="Um eine neue Web-Subdomain zu konfigurieren, müssen Sie die auf der Journey Optimizer-Benutzeroberfläche angezeigten Adobe-Nameserver-Informationen kopieren und in Ihre Domain-Hosting-Lösung einfügen, um den entsprechenden DNS-Eintrag zu generieren. Sobald die Prüfungen erfolgreich waren, kann die Subdomain verwendet werden, um Inhalte aus der Experience Manager Assets Essentials-Bibliothek zu veröffentlichen."
...
1. Geben Sie die zu delegierende Subdomain an.
   ...
   >[!CAUTION]
   >
   >Sie können keine vorhandene Web-Unterdomäne verwenden.
   >
   ...
   >[!NOTE]
   >
   >Es werden Subdomains mit mehreren Ebenen (derselben übergeordneten Domain) unterstützt. Sie können beispielsweise &quot;web.marketing.yourcompany.com&quot;verwenden.
...
1. Nachdem die Subdomain-Zuweisung übermittelt wurde, wird die Subdomain in der Liste mit dem Status **[!UICONTROL Verarbeitung läuft]** angezeigt. Weiterführende Informationen zum Status von Subdomains finden Sie in [diesem Abschnitt](../configuration/about-subdomain-delegation.md#access-delegated-subdomains).<!--Same statuses?-->
   ...
   >[!NOTE]
   >
   >Bevor Sie diese Subdomain zum Senden von Webnachrichten verwenden können, müssen Sie warten, bis Adobe die erforderlichen Prüfungen durchführt, die bis zu 4 Stunden dauern können.

1. Sobald die Prüfungen erfolgreich abgeschlossen sind, erhält die Subdomain den Status **[!UICONTROL Erfolg]**. Es kann verwendet werden, um Webkanaloberflächen zu erstellen.
