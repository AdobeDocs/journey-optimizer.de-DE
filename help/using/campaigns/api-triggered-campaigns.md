...
Mit [!DNL Journey Optimizer] können Sie Kampagnen erstellen und diese dann in einem auf einem Benutzer-Trigger basierten externen System über eine [REST-API zur Ausführung interaktiver Nachrichten](https://developer.adobe.com/journey-optimizer-apis/references/messaging/#tag/execution) auslösen. Auf diese Weise können Sie diversen Bedarf an Marketing- und Transaktionsnachrichten abdecken, wie z. B. das Zurücksetzen von Passwörtern, OTP-Token usw.
...
1. Wählen Sie einen der unterstützten Kanäle und die zugehörige Kanalkonfiguration aus, die zum Senden Ihrer Nachricht verwendet werden soll, und klicken Sie auf **[!UICONTROL Erstellen]**.
...
Um diese Daten in Ihren Kampagnen verwenden zu können, müssen Sie sie an die API-Payload übergeben und mithilfe des Personalisierungseditors zu Ihrer Nachricht hinzufügen.  Verwenden Sie dazu die Syntax `{{context.<contextualAttribute>}}`, wobei `<contextualAttribute>` mit dem Namen der Variablen in Ihrer API-Payload, die die zu übergebenden Daten enthält, übereinstimmen muss.
