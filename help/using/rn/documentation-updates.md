...
* Die Dokumentation **Nachrichteninhalt exportieren** wurde um eine **Häufig gestellte Fragen zum Nachrichtenexport** (personalisierte Inhalte, Bilder und Medien, getrackte Links, personenbezogene Daten, Kundenbindung, Anwendungsfälle usw.) und **Beispiele für exportierte JSON-** für SMS und E-Mail erweitert. [Weitere Informationen](../configuration/message-export.md)

* Eine neue Seite **AJO-**-Export-Schema dokumentiert jedes Feld im AJO-Nachrichtenexport-Datensatz mit Datentypen und einer Hierarchie für die exportierte E-Mail- und SMS-Payload. [Weitere Informationen](../configuration/message-export-schema.md)
...
* Die **Nutzen von Fragmenten in Entscheidungsrichtlinien** Dokumentation enthält jetzt Leitplanken für den **E-Mail**-Kanal: **[!UICONTROL Inhalt simulieren]** zeigt keine Ausdrucksfragmente aus dem Entscheidungselement an, während **[!UICONTROL Testversand durchführen]** und aktivierte Kampagnen dies tun. Auf der Seite wird auch angegeben **[!UICONTROL dass „Visuelle Fragmente]** keinem Entscheidungselement zugewiesen werden können. Nur **Ausdrucksfragmente** werden in diesem Kontext unterstützt. [Weitere Informationen](../experience-decisioning/fragments-decision-policies.md)

## März 2026 {#march-2026}

* Die Dokumentation für **Vorschau von Code-basierten Erlebnissen mit Experience Decisioning** stellt jetzt klar, dass **[!UICONTROL Inhalt simulieren]** nur die Inhaltsvorschau ist. Kontextdaten aus Live-Edge-Anfragen werden in der Authoring-Vorschau nicht simuliert. [Weitere Informationen](../code-based/test-code-based.md#preview-code-based)

* Die **Verwenden von Adobe Experience Platform** Daten“ wurde aktualisiert: Die Leitplanken geben nicht mehr an, dass Datensatzsuchen nicht verkettet werden können. Dies spiegelt das aktuelle Produktverhalten wider. [Weitere Informationen](../data/lookup-aep-data.md)

* Die **Profil aktualisieren**-Aktivitätsdokumentation wurde aktualisiert, um die Unterstützung für die Aktualisierung von bis zu fünf Profilattributen in einer einzigen Aktion zu dokumentieren. [Weitere Informationen](../building-journeys/update-profiles.md)

* Die **Aktivität „Zielgruppe lesen** und **Journey-Eigenschaften** wurde aktualisiert, um den 91-tägigen Journey-Lebenszyklus für immer wiederkehrende Journey zu verdeutlichen. Der Zeitplanabschnitt bestätigt jetzt explizit, dass wiederkehrende Journey ohne Enddatum nach 91 Tagen aktiv bleiben. Die häufig gestellten Fragen zur globalen maximalen Wartezeit wurden erweitert, um die 91-Tage-Profil-TTL vom 91-Tage-Reporting-Fenster zu unterscheiden. [Weitere Informationen](../building-journeys/read-audience.md#schedule)

* Die **Datensatzsuche**-Aktivitätsdokumentation wurde aktualisiert, um klarzustellen, dass der Lookup-Schlüssel im erweiterten Modus konfiguriert werden muss, damit die `@datasetLookup{}`-Syntax in nachgelagerten Bedingungsaktivitäten funktioniert. Es wurde ein Abschnitt zur Fehlerbehebung mit Anleitungen zur Behebung des Fehlers „Datensatz-Suche nicht gefunden“ hinzugefügt. [Weitere Informationen](../building-journeys/dataset-lookup.md#troubleshooting)

* Die **Datums-** Zeitfunktionen“-Dokumentation wurde mit einem neuen Beispiel aktualisiert, das zeigt, wie ein Zeitstempel aus einem Kontextereignisattribut formatiert wird, einschließlich der `toDateTime()`, der Backtick-Syntax für numerische Ereignis-IDs und einem allgemeinen Fehlerhinweis für den PQL-Fehler „Nicht übereinstimmende Eingabe“. [Weitere Informationen](../personalization/functions/dates.md#format-date)

* Die Dokumentation **Leitplanken und Einschränkungen für** und **Erste Schritte mit Quellen-Connectoren** wurde aktualisiert, um klarzustellen, dass für die dateibasierte Änderungsdatenerfassung das Feld `_change_request_type` erforderlich ist und seine Werte `u` (upsert) oder `d` (delete), nicht in Großbuchstaben, lauten müssen. [Weitere Informationen](../orchestrated/guardrails.md)

* Die Dokumentation **Links hinzufügen und Nachrichten verfolgen** wurde mit Anleitungen aktualisiert, wie Tracking-IDs (urlID) generiert werden: Eine eindeutige urlID wird nur zugewiesen, wenn sowohl die URL als auch die Kennzeichnung eindeutig sind. Um dieselbe URL über mehrere E-Mails hinweg (oder mehrmals in einer E-Mail) zu verfolgen, müssen Benutzende für jede ähnliche URL eine eindeutige Kennzeichnung verwenden. Andernfalls können [!DNL Journey Optimizer] nicht ermitteln, auf welchen Link geklickt wurde. [Weitere Informationen](../email/message-tracking.md#track-across-multiple-emails)

* Die **Testprofile erstellen** wurde mit einem wichtigen Hinweis zu den Identitätsdeskriptoranforderungen aktualisiert: Wenn ein Datensatz gelöscht und neu erstellt wird, muss das Schema den richtigen Identitätsdeskriptor für das primäre Identitätsfeld beibehalten. Ohne sie werden aufgenommene Profile nicht als `testProfile = true` gekennzeichnet, selbst wenn die Aufnahme erfolgreich abgeschlossen wurde. Eine Checkliste zur Fehlerbehebung wurde hinzugefügt. [Weitere Informationen](../audience/creating-test-profiles.md)

* Die **Audience lesen**-Aktivität wurde aktualisiert, um klarzustellen, dass eine **Geschäftsereignis**-Aktivität eine Ausnahme von der Regel darstellt, dass „Audience lesen“ die erste Aktivität auf einer Journey sein muss. Es wurde auch ein Hinweis zur Aktivität **Optimieren** als erweiterte Alternative zur Steuerung der Zielgruppen-Zielgruppenbestimmung hinzugefügt. [Weitere Informationen](../building-journeys/read-audience.md)

* **Senden mit Schüben** in Journey ist jetzt allgemein verfügbar. Die Markierung Eingeschränkte Verfügbarkeit wurde aus der Dokumentation entfernt. [Weitere Informationen](../building-journeys/send-using-waves.md)

* Die **Jump**-Aktivitätsdokumentation wurde um den neuen Abschnitt zur Entwurfsstrategie erweitert - **Bite-size Sub-Journey** - in dem erläutert wird, wie komplexe End-to-End-Flüsse in kleinere, fokussierte Sub-Journey unterteilt werden, die über die Jump-Aktivität verbunden sind. [Weitere Informationen](../building-journeys/jump.md#jump-strategy)

* Die **Tags**-Dokumentation wurde mit Anleitungen zur Verwendung von Tag-Kategorien als Alternative zu komplexen Benennungskonventionen aktualisiert. In einem neuen Abschnitt wird erläutert, wie Tag-Kategorien für die skalierbare Journey-Verwaltung eingerichtet werden. [Weitere Informationen](../building-journeys/tags.md)

* Die **Über Datenquellen**-Dokumentation enthält jetzt einen neuen Abschnitt, der den Anwendern bei der Auswahl zwischen drei Datenzugriffsstrategien hilft: Zugriff auf externe Daten über benutzerdefinierte Aktionen, Verwendung eines Datensatzes, der nicht für Profil aktiviert ist, oder Verwendung eines profilaktivierten Datensatzes. Jede Option wird mit Kompromissen und empfohlenen Anwendungsfällen beschrieben. [Weitere Informationen](../datasource/about-data-sources.md#data-access-strategy)

* Die Dokumentation **Design von Push** Benachrichtigungen“ wurde mit einem Hinweis aktualisiert, der das Verhalten universeller Links in iOS verdeutlicht: Wenn die Benachrichtigungs-URL als universeller Link registriert ist, wird die zugehörige App unabhängig von der ausgewählten Web-URL-Aktion geöffnet. Es wurden Anleitungen hinzugefügt, wie das Öffnen eines Browsers erzwungen werden kann. [Weitere Informationen](../push/design-push.md)
...
* Die Dokumentation zur **Adobe Experience Platform** Datenquelle“ wurde mit dem Hinweis aktualisiert, dass in der Data Source-Konfiguration nur XDM Individual Profile-basierte Schemas unterstützt werden. [Weitere Informationen](../datasource/adobe-experience-platform-data-source.md)
...
* Die Dokumentation **Entscheidungsrichtlinien erstellen** und **Entscheidungsrichtlinien in Nachrichten verwenden** wurde für E-Mails aktualisiert: In einem Hinweis wird jetzt erklärt, dass, wenn dasselbe Angebot durch mehr als eine Entscheidungsrichtlinie im E-Mail-Textkörper ausgewählt werden kann, das Modul Angebote dedupliziert (jede Platzierung erhält ein anderes Angebot). Um dasselbe Angebot in mehreren Platzierungen anzuzeigen (z. B. Kopf- und Fußzeile), verwenden Sie **Entscheidungsausgabe wiederverwenden**. [Weitere Informationen](../experience-decisioning/create-decision-policy.md)
...
* Die Dokumentation **Suche nach Erlebnisereignissen in Journey** wurde mit der Zeitleiste zur Einstellung aktualisiert: Ab dem 1. April 2026 haben Organisationen, die in den letzten 90 Tagen keine Erlebnisereignisattribute in Journey-Ausdrücken verwendet haben, keinen Zugriff mehr auf diese Funktion. Die häufig gestellten Fragen konzentrieren sich jetzt auf die Zeitleiste der Außerkraftsetzung und darauf, wer betroffen ist. Die Seite mit dem Schema des Erlebnisereignisses wurde mit einem direkten Link zu alternativen Ansätzen abgestimmt. [Weitere Informationen](../building-journeys/exp-event-lookup.md)

* Die **Decisioning**-Dokumentation wurde für die **Datensatzsuche** mit Adobe Experience Platform-Daten aktualisiert: Die Schutzmaßnahme für unterstützte Kanäle gibt jetzt an, dass die Datensatzsuche für alle Kanäle funktioniert, für die Decisioning verfügbar ist (Code-basiertes Erlebnis, E-Mail, Push, SMS und die Aktivität „Inhaltsentscheidung“ in Journey). Eingeschränkte Verfügbarkeit und öffentliche Hinweise zu Beta wurden aus den Seiten mit Entscheidungsregeln, Rangfolgeformeln und Entscheidungselementen entfernt. [Weitere Informationen](../experience-decisioning/aep-data-exd.md)
...
* Die Dokumentation zum Journey-Probelauf wurde präzisiert: Die Schrittereignisattribute `inDryRun` und `dryRunID` jetzt dokumentieren, dass sie die `true`-/Instanz-ID zurückgeben, wenn sie sich im Probelauf-Modus befinden, und `null` für Test- oder Live-Journey. Die Anleitung zum Ausschluss von Schrittereignissen mit Probelauf in Berichtsabfragen wurde entsprechend aktualisiert. [Weitere Informationen](../building-journeys/journey-dry-run.md)
...
* Die Seite Web-Push-Konfiguration ist jetzt in der Dokumentation verfügbar. [Weitere Informationen](../push/push-configuration-web.md)
...
* Die SMS-Webhook-Dokumentation wurde aktualisiert: Der Twilio-Webhook-Inhalt wurde entfernt. [Weitere Informationen](../sms/sms-webhook.md)

* Die **Konvertieren von Bildern in Inhaltsvorlagen** wurde um erweiterte Leitplanken und Empfehlungen, gängige Anwendungsfälle und klarere Anleitungen zum Konvertieren von Bilddesigns in bearbeitbare HTML-Inhaltsvorlagen erweitert. Außerdem wird erwähnt, dass Sie jetzt ein Design als Eingabe für die Konvertierung verwenden können. [Weitere Informationen](../content-management/image-to-html.md)

* Die Dokumentation zur Decisioning-Migrations-API wurde aktualisiert. [Weitere Informationen](../experience-decisioning/decisioning-migration-api.md)

* Die **Inhaltsentscheidung**-Aktivität ist jetzt allgemein verfügbar. Die Seite mit der Aktivität Inhaltsentscheidung wurde mit einem Abschnitt zu Entscheidungsdaten aktualisiert, die in Schrittereignissen verfügbar sind. [Weitere Informationen](../building-journeys/content-decision.md)

* Links zur Dokumentation zur Herausforderung bezüglich des Treueprogramms wurden zum Abschnitt Herausforderungen im Treueprogramm hinzugefügt (Erste Schritte, Erstellen von Herausforderungen, Erstellen von Aufgaben, Zugriff auf Herausforderungen im Treueprogramm). [Weitere Informationen](../loyalty-challenges/get-started.md)

* Die Informationen zu den unterstützten Kanälen in der Dokumentation zum Assistenten zur Kampagnenerstellung wurden korrigiert. Die FAQ-Seiten zu den ersten Schritten mit Kanälen und orchestrierten Kampagnen wurden entsprechend aktualisiert. [Weitere Informationen](../campaigns/get-started-with-campaigns.md)

* Die Dokumentation zu Berechtigungen wurde in Bezug auf **Berechtigungen vom Typ Journey** Verwalten und **Genehmigen“**. [Weitere Informationen](../administration/ootb-permissions.md)

* Die Dokumentation zu Integrationen mit AEM (Adobe Experience Manager) wurde mit überarbeiteten Namen aktualisiert (dynamische Inhalte von AEM und AEM-Fragmente). [Weitere Informationen](../integrations/aem-fragments.md)

* Der Ausschlussliste wurde ein neuer Ausschlussgrund hinzugefügt: **UnsubscribeLinkNotValid** (Fehlercode 050081). Dieser Ausschluss wird erzeugt, wenn die Betrefflänge von „list-unsubscribe mailTo“ größer als das RFC-Limit von 998 Zeichen ist. [Weitere Informationen](../reports/exclusion-list.md)

* Die Dokumentation zur Hilfsfunktion formatDate wurde um den Hinweis erweitert, dass die Funktion einen Datums-/Uhrzeitfeldtyp (keine Zeichenfolge) erfordert, sowie um mehrere Beispiele: Formatieren eines Datums-/Uhrzeitfelds, Konvertieren einer Zeichenfolge in Datum zuerst, Vollständiges Datum mit Tagesname, Dynamisches Datum aus Systemzeit und Wochentagsformat einschließlich Kleinschreibung. [Weitere Informationen](../personalization/functions/dates.md#format-date)

* Die E-Mail-Dokumentation zur Textversion wurde um umfassende Anleitungen zu Anwendungsfällen erweitert, einschließlich Entscheidungskriterien für die Verwendung von benutzerdefiniertem Klartext im Vergleich zur automatischen Synchronisierung, praktische Beispiele mit realen Szenarien und einen FAQ-Abschnitt mit häufigen Fragen. [Weitere Informationen](../email/text-version-email.md#when-to-use)

* Die Dokumentation zu E-Mail-Designer-Designs wurde mit Informationen zu den Einschränkungen bei der Unterstützung von Web-Schriftarten und zur Bedeutung von Fallback-Schriftarten aktualisiert. [Weitere Informationen](../email/apply-email-themes.md#themes-guardrails)

* Es wurde eine Einschränkung zur Hilfedokumentation zu Ausführungsmetadaten hinzugefügt, um klarzustellen, dass Metadaten nicht für Profile erfasst werden, die von der Aktion ausgeschlossen sind. [Weitere Informationen](../personalization/functions/helpers.md#execution-metadata)

* Die Dokumentation mit den Code-basierten Implementierungsbeispielen wurde aktualisiert und enthält jetzt das Token-Feld in propositionAction , um ein genaues Tracking und die Attribution in Decisioning zu ermöglichen. [Weitere Informationen](../code-based/code-based-implementation-samples.md#client-side-how)

* Es wurde ein Hinweis zur Dokumentation URL-Tracking und Abmelde-Liste hinzugefügt, um klarzustellen, dass die Reihenfolge der URL-Tracking-Parameter, die an URLs angehängt werden, zufällig ist und nicht gesteuert werden kann. [Weitere Informationen](../email/url-tracking.md)

## Januar 2026 {#january-2026}

* Die Dokumentation zum Lizenznutzungs-Dashboard wurde mit aktualisierten Anleitungen zu **Engageable Profiles“ ergänzt** einschließlich Definitionsdetails und Anleitungen zur Fehlerbehebung. [Weitere Informationen](../audience/license-usage.md#what-is-engageable-profile)

* Es wurde ein Hinweis zur E-Mail-Dokumentation zu Designer-Designs hinzugefügt, um die Einschränkungen bei der Unterstützung von Web Fonts zu verdeutlichen. [Weitere Informationen](../email/apply-email-themes.md#themes-guardrails)
...
* Neue Abschnitte zu Einschränkungen und Best Practices wurden der Dokumentation zu visuellen Fragmenten hinzugefügt, um Benutzende vor nicht unterstütztem Verschachtelungen von Fragmenten mit dynamischen Inhalten innerhalb anderer entsperrter Fragmente mit dynamischen Inhalten zu warnen. Die Anleitung enthält Schritte zur Fehlerbehebung bei Problemen mit dem Kompatibilitätsmodus und Empfehlungen für die richtige Gestaltung der E-Mail-Struktur. [Weitere Informationen](../email/use-visual-fragments.md#fragment-dynamic-content)
