...
title: Verwenden und Konfigurieren von Deep-Links in E-Mail- und SMS-Nachrichten
description: Erfahren Sie, wie Sie Deep-Links zu E-Mail- und SMS-Inhalten hinzufügen und wie Sie die Deep-Link-Handhabung in iOS- und Android-Apps implementieren.
feature: Email, SMS
...
>[!NOTE]
>
>[!DNL Adobe Journey Optimizer] unterstützt Deep-Linking für iOS und Android mithilfe von getrackten URLs (`/ee/v1/mclick/*`), um Kompatibilität und Klick-Tracking sicherzustellen.

## Erstellen von Deep-Links {#authoring}

>[!CAUTION]
>
>Deep-Links funktionieren nur, wenn Sie die Schritte [Konfiguration](#configuration) und [Mobile-App-](#mobile-implementation) auf dieser Seite abgeschlossen haben.
...
>[!NOTE]
>
>Dieser Abschnitt gilt für die Verwendung **universellen Links (iOS)** und **App-Links (Android)** (HTTPS-basierte Deep-Links).
...
>[!IMPORTANT]
>
>Deep-Linking über die Adobe-Infrastruktur gilt, wenn das Linktracking für Ihre Nachricht aktiviert ist - in den E[Mail-Tracking](message-tracking.md#enable-tracking)Einstellungen oder im Abschnitt **[!UICONTROL Aktionstracking]** für SMS-Kampagnen. Getrackte Deep-Link-Klicks verwenden URLs unter `/ee/v1/mclick/*`, die von Adobe gehostet und aufgelöst werden.
>
...
In diesem Abschnitt wird erläutert, wie Sie mobile Deep-Links mit [!DNL Adobe Journey Optimizer] implementieren, sodass in einem typischen **HTTPS**-Setup (universelle Links und App-Links) eine einzelne URL:
...
* **Vermeiden Sie vertrauliche Daten in URLs**: Fügen Sie keine Geheimnisse oder persönlichen Daten direkt in die Deep-Link-URL ein.
...
+++ Die App wird nicht geöffnet, wenn ich auf den Deep-Link tippe.
...
Ja. Wenn Sie für jede delegierte Subdomain eine Deep-Verknüpfung wünschen, fordern Sie die AASA- und `assetlinks.json`-Konfiguration für jede Subdomain an, die die Funktion unterstützen soll.

+++

+++ Sollte die von mir konfigurierte URL ein Deep-Linking-Format verwenden (z. B. `appname://path`)?
